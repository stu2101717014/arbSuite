...
public class NSUpdateReceiver {
    ...
    @Autowired
    public NSUpdateReceiver(@Qualifier("nsuQueue") Queue qu, NamesSimilaritiesService namesSimilaritiesService, GsonService gsonService) {
        this.qu = qu;
        this.namesSimilaritiesService = namesSimilaritiesService;
        this.gsonService = gsonService;
    }

    @RabbitListener(queues = "#{nsuQueue.getName()}")
    public String getNameSimilarity(String message) {
        ...
        List<NamesSimilaritiesDAO> resList = this.namesSimilaritiesService.saveAllAndFlushNamesSimilarities(daosList);
        return this.gsonService.getGson().toJson(resList);
    }
}
